    ...
    public class RefreshTokenMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context, IAuthService authService)
        {
            ...
            if (!string.IsNullOrEmpty(refreshToken))
            {
                ...
                var parsed = Helpers.AuthHelper.ParseUserIdFromToken(accessToken);
                if (!parsed.isAuth)
                {
                    var result = await authService.RefreshToken(refreshToken);
                    if (result.Success && !string.IsNullOrEmpty(result.newAccessToken))
                    {
                        context.Response.Cookies.Append("accessToken", result.newAccessToken, new CookieOptions
                        {
                            HttpOnly = true,
                            Secure = true,
                        ...
                        {
                            context.Response.Cookies.Append("refreshToken", result.newRefreshToken, new CookieOptions
                            {
                                HttpOnly = true,
                                Secure = true,
                                SameSite = SameSiteMode.None,
